...
class Driver{
    public static void main(String[] args) throws IOException {
        ...
        if(HiddenMain(test1)==161 && HiddenMain(test2)==48){
    ...
    //Not 79748299, 73243482, 25292714, 161877942, 161289189
    ...
    public static ArrayList<String>fileBreakdown(ArrayList<String> data){
        ArrayList<String> temp = new ArrayList<>();
        //...code goes here to break up the mul(#,#) into their own seperate lines.
        for(String d : data){
            String p = d;
            p=p.replace("m", "\nm");
            p=p.replace(")",")\n");
    ...
    public static ArrayList<String> stringToArrayConvertV2(String string){
        ...
        for(int i=0;i<hope.size();i++){
            ...
            Pattern pat2 = Pattern.compile("do\\(\\)");
            Matcher mat2 = pat2.matcher(hope.get(i));
            Pattern pat3 = Pattern.compile("don't\\(\\)");
            Matcher mat3 = pat3.matcher(hope.get(i));
            if(matcher.find()){
                String data = matcher.group();
                hope.set(i, data);
            }
            else if(mat2.find()){
                String data = mat2.group();
                hope.set(i, data);
            }
            else if(mat3.find()){
                String data = mat3.group();
                hope.set(i, data);
            }else{
                hope.set(i, "-1");
            }
    ...
    public static int arrayToTotal(ArrayList<String> data){
        ...
        for(String d:data){
            ...
            else if(d.contains("don't()")){
                calc = false;
            }
            else if(d.contains("mul(")&&calc){
            d=d.replace("mul(", "");
            d=d.replace(")", "");
            String[] datS = d.split(",");
